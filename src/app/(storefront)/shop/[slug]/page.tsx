...
export default function VendorShopPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const vendor = useMemo(() => {
    // Tìm vendor theo slug, hỗ trợ cả trường hợp slug có thể bị sai lệch chữ hoa/thường
    return vendors.find(v => v.storeSlug?.toLowerCase() === slug.toLowerCase());
  }, [vendors, slug]);

  // Thiết lập branding cho Header/Footer khi vào shop
  useEffect(() => {
    if (vendor) {
    ...
    return () => {
      // Xóa branding khi rời khỏi shop
    ...
    <div className="container mx-auto px-4 py-12 animate-in fade-in duration-500">
      ...
      <div className="bg-card/40 border border-white/5 rounded-[2.5rem] p-8 md:p-12 mb-12 shadow-2xl relative overflow-hidden group">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center relative z-10">
          <div className="h-32 w-32 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center shadow-inner shrink-0 overflow-hidden">
             {vendor.storeLogo ? (
               <Image src={vendor.storeLogo} alt={vendor.storeName} width={128} height={128} className="object-cover h-full w-full" />
             ) : (
      ...
      <div className="space-y-8">
        <div className="flex justify-between items-end border-b border-white/5 pb-4">
          <h2 className="text-2xl font-bold font-headline italic uppercase tracking-tighter">SẢN PHẨM TỪ SHOP ({shopProducts.length})</h2>
        </div>
        ...
        {shopProducts.length > 0 ? (
          ...
          <div className="text-center py-32 bg-muted/10 rounded-[2.5rem] border border-dashed border-white/10">
            ...
            <p className="text-muted-foreground italic">Shop hiện chưa có sản phẩm nào được đăng bán hoặc đang chờ kiểm duyệt.</p>
          </div>
        )}
      </div>
    </div>
